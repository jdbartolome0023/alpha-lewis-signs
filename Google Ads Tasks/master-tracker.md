...
- **Sep 17 2026:** Traced the mystery Google tag found in theme.liquid (see earlier entry) to Signee's SEO provider: they already run an active GA4 property (account "Signee" 407497418, property 553528757, Measurement ID G-8LB953WBW4, actively receiving traffic). Josette got GA4 access to it directly. Decision: consolidate onto this existing property instead of running two separate GA4 setups. Updated the GTM container's "GA4 Configuration - Signee" tag to point at G-8LB953WBW4 (was G-16K9H45R2V) and saved as a new workspace version, not yet published/submitted. The GA4 property built earlier today (account "Signee" 408499053) is now redundant, left dormant, not deleted. Still to do before installing the GTM snippet on the site: remove the SEO's hardcoded gtag.js block from theme.liquid so only GTM fires the tag, then publish/submit the GTM container.
